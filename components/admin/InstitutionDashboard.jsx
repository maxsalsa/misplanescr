...
export default function InstitutionAdminDashboard({
  ...
  const handleRevoke = (id) => {
    if (
      !confirm(
        "¿Seguro que desea desvincular a este docente? Se liberará 1 licencia.",
      )
    )
      return;
      ...
      <div className="flex justify-between items-end mb-8 border-b pb-4">
        <div>
          ...
          <p className="text-slate-500 font-bold flex gap-2 items-center">
            <ShieldCheck size={18} className="text-success" /> Panel de Control
            de Licencias (B2B)
          </p>
        </div>
        <div className="text-right">
          ...
          <div className="text-4xl font-black text-primary">
            {current}{" "}
            <span className="text-lg text-slate-400 font-normal">/ {max}</span>
          </div>

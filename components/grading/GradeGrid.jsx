  ...
  const handleLevelClick = async (studentId, criterionId, level) => {
    ...
    setGridData((prev) =>
      prev.map((row) => {
        ...
        const existingIdx = newDetails.findIndex(
          (d) => d.criterionId === criterionId,
        );
      ...
      <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
        <div>
          <h2 className="text-lg font-bold text-slate-800">
            Speed Grader: {activityTitle}
          </h2>
          <p className="text-sm text-slate-500">
            Haz clic en los niveles para calificar.
          </p>
        </div>
        <div className="flex gap-2 text-xs font-medium">
          <span className="flex items-center gap-1 px-2 py-1 bg-red-100 text-red-700 rounded-md">
            🔴 Inicial (1)
          </span>
          <span className="flex items-center gap-1 px-2 py-1 bg-yellow-100 text-yellow-700 rounded-md">
            🟡 Intermedio (2)
          </span>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-slate-500 uppercase bg-slate-50">
            <tr>
              <th className="px-6 py-4 font-bold text-slate-700 w-64 sticky left-0 bg-slate-50 z-10">
                Estudiante
              </th>
              {taxonomies.map((tax) => (
                <th
                  key={tax.id}
                  className="px-4 py-4 text-center min-w-[140px] border-l border-slate-200"
                >
              ...
              <th className="px-6 py-4 text-right font-bold text-slate-700 w-32 bg-slate-50 sticky right-0 z-10 border-l border-slate-200">
                Nota Final
              </th>
            </tr>
          </thead>
              ...
              >
                ...
                {taxonomies.map((tax) => {
                  ...
                  return (
                    <td
                      key={tax.id}
                      className="px-4 py-4 text-center border-l border-slate-100"
                    >
                      <div className="flex justify-center gap-1">
                        {[1, 2, 3].map((lvl) => (
                          <button
                            key={lvl}
                            onClick={() =>
                              handleLevelClick(row.student.id, tax.id, lvl)
                            }
                            className={cn(
                              ...
                              lvl === 1 && currentLevel === 1
                                ? "bg-red-500 text-white shadow-md shadow-red-200"
                                : "bg-slate-100 text-slate-300 hover:bg-red-100 hover:text-red-400",
                              lvl === 2 && currentLevel === 2
                                ? "bg-yellow-500 text-white shadow-md shadow-yellow-200"
                                : "bg-slate-100 text-slate-300 hover:bg-yellow-100 hover:text-yellow-400",
                              lvl === 3 && currentLevel === 3
                  ...
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {row.finalGrade < 65 && (
                      <AlertTriangle
                        ...
                        title="Alerta Temprana Detectada"
                      />
                    )}
                  </div>
                </td>

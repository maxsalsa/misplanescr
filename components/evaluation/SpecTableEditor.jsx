    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <tbody className="divide-y divide-slate-100">
            {rows.map((row, i) => (
                <tr key={i} className="hover:bg-blue-50/50 transition-colors group">
                    <td className="p-2"><input className="w-full bg-transparent border-none focus:ring-0 font-medium text-slate-700" value={row.obj} onChange={(e)=>update(i,"obj",e.target.value)} placeholder="Descripción..."/></td>
                    <td className="p-2">
                        <select className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-xs" value={row.cognitive} onChange={(e)=>update(i,"cognitive",e.target.value)}>
                            <option>Conocimiento</option><option>Comprensión</option><option>Aplicación</option><option>Análisis</option>
                        </select>
                    </td>
                    <td className="p-2">
                        <select className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-xs" value={row.type} onChange={(e)=>update(i,"type",e.target.value)}>
                            <option>Sel. Única</option><option>Resp. Corta</option><option>Pareo</option><option>Desarrollo</option>
                        </select>
                    </td>
      ...
      {totals.points !== target && (
        <div className="bg-amber-50 text-amber-700 text-xs p-2 text-center border-t border-amber-100 flex justify-center items-center gap-2">
            <AlertTriangle size={14}/> La tabla está desbalanceada. Ajuste los puntos hasta llegar a {target}.
        </div>
      )}
      {totals.points === target && (
        <div className="bg-emerald-50 text-emerald-700 text-xs p-2 text-center border-t border-emerald-100 flex justify-center items-center gap-2">
            <CheckCircle2 size={14}/> ¡Balance Perfecto! Tabla lista para impresión oficial.
        </div>
      )}
    </div>
  );

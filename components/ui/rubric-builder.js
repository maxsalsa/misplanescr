...
export function RubricBuilder({ onChange }) {
  ...
  const addCriterion = () => {
    setCriteria([
      ...criteria,
      { id: Date.now(), name: "", points: 5, description: "" },
    ]);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {criteria.map((c, index) => (
          <div
            key={c.id}
            className="flex gap-3 items-start bg-slate-50 p-3 rounded-lg border border-slate-200 group"
          >
            ...
            <div className="flex-1 space-y-2">
              <div className="flex gap-2">
                ...
                <input
                  ...
                  value={c.points}
                  onChange={(e) =>
                    updateCriterion(c.id, "points", e.target.value)
                  }
                />
              </div>
              <textarea
            ...
            <button
              ...
              className="btn btn-ghost btn-sm text-slate-400 hover:text-red-500 mt-1"
            >
              <Trash2 size={16} />
            </button>
          </div>
      ...
      <button
        ...
        type="button"
        className="btn btn-sm btn-outline w-full border-dashed border-slate-300 text-slate-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-300"
      >

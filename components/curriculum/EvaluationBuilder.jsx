...
export default function EvaluationBuilder({ curriculumModule }) {
  ...
  const renderHeader = () => {
    ...
    return (
      <div
        className={`p-4 rounded-t-xl border-b ${color} flex justify-between items-center`}
      >
        <div className="flex items-center gap-3">
          {icon}
          <div>
            <h3 className="font-bold text-lg">
              {specialty} <span className="text-sm opacity-70">({level})</span>
            </h3>
        ...
        <div className="mt-4 space-y-3">
          <div className="relative">
            <div className="absolute top-0 right-0 z-10 p-2">
              <span className="badge badge-accent badge-outline text-[10px] font-mono">
                LABORATORIO 3.0
              </span>
            </div>
            <CodeMockup
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {lo.evaluation_component?.evidence_description ||
                    "Script ejecutable o Repositorio"}
                </p>
              </div>
            </div>
          ...
          <div className="px-4 py-2 bg-yellow-50 text-yellow-800 text-xs rounded border border-yellow-100 flex items-start gap-2">
            ...
            <span>
              <b>Firewall Normativo:</b> Se detectó contenido de
              &quot;Ciberseguridad&quot;. No cargue credenciales reales en el
              código.
            </span>
          </div>
        </div>
    ...
    if (uiRenderer === "SOFT_SKILLS") {
      return (
        <div className="mt-4">
          <RoleplayCard
            scenario={
              lo.evaluation_component?.description ||
              lo.dua_strategy ||
              "Realizar simulación de la situación planteada."
            }
          ...
          <div className="mt-4 bg-white p-4 rounded-lg border border-slate-200">
            <h4 className="font-bold text-slate-700 mb-2 flex items-center gap-2">
              <CheckCircle size={14} className="text-green-500" /> Criterios de
    ...
    if (uiRenderer === "ACADEMIC" || !uiRenderer) {
      return (
        <div className="mt-4 p-4 rounded-lg bg-indigo-50 text-indigo-900 text-sm border border-indigo-100">
          <p className="mb-4">
            <b>Resultado de Aprendizaje:</b> {lo.lo_text}
          </p>
          {/* Mock Logic for Academic */}
          {lo.lo_text?.includes("Exo") ||
          lo.lo_text?.includes("Quím") ||
          lo.specialty?.includes("Cienc") ? (
            <LabSimulation title="Reacción Química Virtual" />
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-white p-4 rounded shadow-sm">
                <h5 className="font-bold text-xs uppercase text-slate-400 mb-2">
                  Análisis de Datos
                </h5>
                <DataGraph />
              </div>
              <div className="bg-white p-4 rounded shadow-sm flex flex-col justify-center">
                ...
                <p className="text-xs text-slate-500 mb-3">
                  Se requiere estructura IMRyD (Introducción, Métodos,
                  Resultados y Discusión).
                </p>
                <button className="btn btn-xs btn-outline">
                  Ver Rúbrica de Informe
                </button>
              </div>
            </div>
          )}

      ...
      <AnimatePresence>
        {isOpen && (
          <motion.aside initial={{x:"-100%"}} animate={{x:0}} exit={{x:"-100%"}} className="fixed inset-0 z-50 bg-slate-900 text-white w-3/4 shadow-2xl md:hidden p-4">
             <div className="flex justify-between mb-8"><h2 className="font-bold text-xl">Menú</h2><button onClick={()=>setIsOpen(false)}><X/></button></div>
             <nav className="space-y-4">
               {MENU.map(i => <Link key={i.href} href={i.href} onClick={()=>setIsOpen(false)} className="flex gap-3 text-lg">{i.icon} {i.name}</Link>)}
             </nav>
             <button onClick={logout} className="mt-8 text-red-400 flex gap-2"><LogOut/> Cerrar Sesión</button>
          </motion.aside>
        )}
      </AnimatePresence>
    </>

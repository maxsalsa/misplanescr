...
export default function GradebookTable({ students, plans, initialGrades }) {
  ...
  const getGradeColor = (score) => {
    if (!score && score !== 0)
      return "bg-slate-50 border-slate-200 text-slate-400"; // Empty
    ...
    <div className="overflow-x-auto border border-slate-200 rounded-xl shadow-sm">
      <table className="table bg-white w-full">
        ...
        <thead className="bg-slate-50 text-slate-700">
          <tr>
            <th className="sticky left-0 bg-slate-50 z-10 w-48 shadow-sm">
              Estudiante
            </th>
            {plans.map((plan) => (
              <th
                key={plan.id}
                className="min-w-[150px] text-center font-medium"
              >
              ...
              >
                ...
                {plans.map((plan) => {
                  ...
                  const colorClass = getGradeColor(
                    currentGrade ? currentGrade.score : null,
                  );
                ...
                <td className="text-center font-bold">
                  <span
                    className={`px-3 py-1 rounded-full text-xs ${getGradeColor(average === "-" ? null : parseFloat(average))}`}
                  >
                    {average}
                  </span>
                </td>

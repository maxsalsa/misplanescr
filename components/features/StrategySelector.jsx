          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden border border-slate-200">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-lg leading-tight">
                    HUD Pedagógico V-Ultra
                  </h3>
                  <p className="text-[10px] text-slate-400">
                    Banco de Estrategias Oficiales (MEP)
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-4 bg-slate-50 border-b border-slate-200 flex gap-2 overflow-x-auto">
              <button
                ...
                className={`btn btn-xs ${filter === "TODOS" ? "btn-neutral" : "btn-ghost"}`}
              >
                Todo
              </button>
              <button
                onClick={() => setFilter("COTIDIANO")}
                className={`btn btn-xs gap-1 ${filter === "COTIDIANO" ? "btn-primary" : "btn-ghost"}`}
              >
                <Book size={12} /> Cotidiano
              </button>
              <button
                onClick={() => setFilter("TAREA")}
                className={`btn btn-xs gap-1 ${filter === "TAREA" ? "btn-secondary" : "btn-ghost"}`}
              >
                <Layout size={12} /> Tareas
              </button>
              <button
                onClick={() => setFilter("PROYECTO")}
                className={`btn btn-xs gap-1 ${filter === "PROYECTO" ? "btn-accent" : "btn-ghost"}`}
              >
                <FlaskConical size={12} /> Proyectos
              </button>
            </div>
                ...
                filteredStrategies.map((strat) => (
                  ...
                  >
                    <div className="card-body p-4">
                      ...
                      {strat.title.includes("Picto") && (
                        <span className="text-[9px] text-purple-600 font-bold mt-1 block">
                          ✨ DUA VISUAL
                        </span>
                      )}
                      {strat.title.includes("Podcast") && (
                        <span className="text-[9px] text-rose-600 font-bold mt-1 block">
                          🎧 DUA AUDITIVO
                        </span>
                      )}
                    </div>

  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6 font-sans">
      <header className="flex justify-between items-center mb-8 bg-slate-900 p-6 rounded-3xl border border-white/5 shadow-2xl">
        <div>
          <h1 className="text-2xl font-black uppercase text-[var(--color-primary-400)]">
            Panel de Control: Suscriptor
          </h1>
          <p className="text-xs text-slate-400">
            Licencia Activa:{" "}
            <span className="text-green-400 font-bold">PROFESIONAL 2026</span>
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setActiveTab("GRUPOS")}
            className={`btn btn-sm ${activeTab === "GRUPOS" ? "btn-primary" : "btn-ghost"}`}
          >
            <Users size={16} /> Grupos
          </button>
          <button
            onClick={() => setActiveTab("NOTAS")}
            className={`btn btn-sm ${activeTab === "NOTAS" ? "btn-primary" : "btn-ghost"}`}
          >
            <Calculator size={16} /> Notas
          </button>
          <button
            onClick={() => setActiveTab("EVIDENCIAS")}
            className={`btn btn-sm ${activeTab === "EVIDENCIAS" ? "btn-primary" : "btn-ghost"}`}
          >
              ...
              <table className="table w-full text-sm">
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <button className="btn btn-xs btn-link text-blue-400">
                        Ver Rúbrica
                      </button>
                    </td>
                  </tr>
        ...
        {activeTab === "EVIDENCIAS" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-[#162534] p-6 rounded-3xl border border-white/5">
              <h3 className="font-bold mb-4 uppercase text-sm">
                Entregas Recientes
              </h3>
              <ul className="space-y-4">
                <li className="flex items-center gap-3 bg-white/5 p-3 rounded-xl border border-white/5 hover:bg-white/10 cursor-pointer transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      10-1 Ciberseguridad • Hace 5 min
                    </p>
                  </div>
                  <div className="ml-auto">
                    <span className="badge badge-info badge-xs text-xs">
                      Juego
                    </span>
                  </div>
                </li>
                <li className="flex items-center gap-3 bg-white/5 p-3 rounded-xl border border-white/5 hover:bg-white/10 cursor-pointer transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      10-1 Ciberseguridad • Hace 1 hor
                    </p>
                  </div>
                  <div className="ml-auto">
                    <span className="badge badge-success badge-xs text-xs">
                      Práctica
                    </span>
                  </div>
                </li>
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 text-center flex flex-col items-center justify-center">
              ...
              <p className="text-slate-400 text-sm max-w-xs mb-6">
                La gestión de evidencias requiere almacenamiento en la nube.
                Actualice su plan para habilitar 50GB de espacio para sus
                estudiantes.
              </p>
              <button className="btn btn-warning w-full max-w-xs font-bold text-black">
                Aumentar Capacidad
              </button>
            </div>
          </div>

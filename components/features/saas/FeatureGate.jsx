...
export function FeatureGate({
  children,
  fallbackMessage = "Esta función es exclusiva para usuarios PRO.",
  showBlur = true,
}) {
  ...
  return (
    <div className="relative group overflow-hidden rounded-xl border border-slate-200">
      {/* Blurry Content */}
      <div
        className={`transition-all duration-500 ${showBlur ? "blur-md opacity-50 grayscale pointer-events-none select-none" : "hidden"}`}
      >
        {children}
      </div>
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/60 backdrop-blur-sm p-6 text-center">
        ...
        <p className="text-slate-600 mb-6 max-w-sm">{fallbackMessage}</p>
        <button

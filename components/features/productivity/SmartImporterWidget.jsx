    ...
    <div className="card bg-base-100 shadow-xl border border-base-300">
      <div className="card-body">
        ...
        <p className="text-sm opacity-70">
          Sube tu lista de estudiantes (Excel/CSV). El sistema limpiará los
          datos automáticamente.
        </p>

        {/* DRAG & DROP ZONE */}
        <div
          className={`border-2 border-dashed rounded-lg p-8 flex flex-col items-center justify-center transition-colors ${file ? "border-primary bg-primary/10" : "border-base-300 bg-base-50"}`}
        >
          {!file ? (
            <>
              ...
              <span className="text-xs text-base-400 mt-1">
        ...
        {/* PREVIEW REPORT */}
        {loading && (
          <div className="loading loading-bars loading-lg mx-auto my-4 text-primary"></div>
        )}
              ...
              <div className="alert alert-warning shadow-sm items-start">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-xs">
                    Atención requerida en {report.logs.length} filas
                  </h3>
                  <ul className="text-xs list-disc pl-4 mt-1 max-h-24 overflow-y-auto">
                    {report.logs.map((L, i) => (
                      <li key={i}>
                        Fila {L.row}: {L.errors.join(", ")}
                      </li>
                    ))}
                  </ul>
                </div>

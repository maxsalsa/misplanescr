...
export default function PlanRenderer({
  ...
  const { encabezado, columnas, mediacion, versionNumber, source, createdAt } =
    activeData;
    ...
    <div className="space-y-8 animate-in fade-in duration-700 font-sans">
      ...
      <div className="flex justify-between items-center bg-slate-50 p-2 rounded-xl border border-slate-100 shadow-sm">
        <div className="flex items-center gap-3 px-2">
          <Badge
            variant="outline"
            className="bg-white gap-1.5 py-1 px-3 shadow-sm border-slate-200 text-slate-600"
          >
      ...
      >
        {/* Decorative Top Bar */}
        <div
          className={`h-2 w-full bg-gradient-to-r from-${themeColor}-500 via-${themeColor}-400 to-${themeColor}-300`}
        />
        ...
        <div className="p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <div
                className={`p-2.5 rounded-lg bg-${themeColor}-50 text-${themeColor}-600`}
              >
                    ...
                    <div key={idx} className="grid grid-cols-1 gap-0">
                      ...
                      <div className="bg-slate-50/80 p-5 border-b border-white">
                        ...
                        {col.criterio && (
                          <Badge
                            variant="outline"
                            className="mt-2 text-[10px] border-slate-300 text-slate-500"
                          >
                            {col.criterio}
                          </Badge>
                        )}
                      ...
                      <div className="p-5 bg-white">
                        ...
                        <ul className="space-y-2">
                          ...
                          {Array.isArray(col.saberes) ? (
                            col.saberes.map((s, i) => (
                              <SaberItem key={i} text={s} color={themeColor} />
                            ))
                          ) : col.saber ? (
                            <SaberItem text={col.saber} color={themeColor} />
                          ) : (
...
function SaberItem({ text, color }) {
  return (
    <li className="flex items-start gap-2 text-sm text-slate-600 leading-snug">
      <span
        className={`mt-1.5 w-1.5 h-1.5 rounded-full bg-${color}-400 flex-shrink-0`}
      />
      <span>{text}</span>
    </li>
  );
...
function MediationCard({ fase, idx, themeColor, readOnly }) {
  ...
  const borderColor = isCierre
    ? "border-l-emerald-500"
    : `border-l-${themeColor}-500`;

  return (
    <Card
      className={`overflow-hidden transition-all hover:shadow-md border-l-4 ${borderColor}`}
    >
...
function IronLawGuard({ content }) {
  if (!content)
    return <span className="text-gray-300 italic text-xs">Sin asignar.</span>;

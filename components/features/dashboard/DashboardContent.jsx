...
export default function DashboardContent({ initialPlans, userRole, userName }) {
  ...
  const toggleContext = () => {
    ...
    toast(
      nuevo === "DIURNA"
        ? "☀️ Modo Diurno Activo"
        : "🌙 Modo Nocturno (Técnico) Activo",
    );
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-slate-50 to-slate-100 p-6 rounded-2xl border border-slate-200">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span
              className={`badge ${contexto === "DIURNA" ? "badge-warning" : "badge-neutral"}`}
            >
        ...
        <div className="flex items-center gap-4 bg-white p-3 rounded-xl shadow-sm border border-slate-100">
          ...
          <button
            ...
            data-tip="Cambiar Contexto Laboral"
          >
        ...
        <div className="bg-slate-900 text-slate-100 p-6 rounded-2xl shadow-xl border border-slate-700">
          <div className="flex items-center gap-3 mb-4">
            ...
            <Link
              ...
              className="ml-auto btn btn-xs btn-outline btn-error"
            >
              Ir a Panel Avanzado →
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button
              onClick={handleAdminAudit}
              className="btn bg-slate-800 border-slate-700 hover:bg-slate-700 text-white gap-2"
            >
            ...
            {initialPlans.length > 0 ? (
              <div className="grid gap-3">
                {initialPlans.map((plan) => (
                  <div
                    key={plan.id}
                    className="p-4 rounded-xl bg-slate-50 border border-slate-100 hover:border-indigo-200 transition-colors flex justify-between items-center group"
                  >
                    <div>
                      <h4 className="font-semibold text-slate-800">
                        {plan.asignatura || "Sin Título"}
                      </h4>
                      <p className="text-sm text-slate-500">
                        {plan.nivel} • {plan.modalidad || "Académica"}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="badge badge-ghost opacity-0 group-hover:opacity-100 transition-opacity">
                        v3.0
                      </span>
              ...
              <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                ...
                <p className="text-slate-500">
                  No hay planes en este contexto.
                </p>
                <Link
        ...
        <div className="space-y-6">
          <div className="card bg-gradient-to-br from-indigo-600 to-purple-700 text-white shadow-xl p-6 relative overflow-hidden">
            ...
            <p className="text-indigo-100 mt-2 relative z-10 text-sm">
              Genera 1 planeamiento de Ciencias para desbloquear la insignia
              &quot;Científico Loco&quot;.
            </p>
          ...
          <div className="card bg-base-100 shadow-xl p-6">
            <h2 className="font-bold mb-4 flex items-center gap-2">
              <Activity size={18} className="text-green-500" /> Actividad
              Reciente
            </h2>

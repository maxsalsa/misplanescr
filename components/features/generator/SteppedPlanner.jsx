...
const SteppedPlanner = memo(function SteppedPlanner({ onGenerate, loading }) {
  ...
  const isFirstGradePeriod1 =
    (data.level?.includes("1") || data.level === "1") &&
    data.modality.includes("Primaria") &&
    data.period === "I";
  ...
  const getFilteredModules = useCallback(() => {
    ...
    const family =
      data.modality === "Educación Técnica" ? "HARD_TECH" : "SOFT_SKILLS";
    return mepModules.filter((m) => m.ui_family === family);
  }, [data.modality, mepModules]);

  const derivedLevels = [
    ...new Set(getFilteredModules().map((m) => m.level)),
  ].sort((a, b) => a - b);
  ...
  const availableSubareas = [
    ...new Set(
      getFilteredModules()
        .filter(
          (m) =>
            m.specialty === data.subject && m.level.toString() === data.level,
        )
  ...
  const currentModule = mepModules.find(
    (m) =>
      m.specialty === data.subject &&
      m.level.toString() === data.level &&
      (data.modality === "Educación Técnica"
        ? m.ui_family === "HARD_TECH"
        : m.ui_family === "SOFT_SKILLS"),
  );

  const availableUnitsForSelection = currentModule
    ? currentModule.learning_outcomes.map((ra) => ra.description)
    : [];
  ...
  const submit = () => {
    ...
    if (data.resourceType === "instrumento_sumativo") {
      ...
      const officialIndicators =
        currentModule?.learning_outcomes?.map((lo) => ({
      ...
      if (officialIndicators.length === 0) {
        return toast.error(
          "Error Normativo: No se encontraron indicadores oficiales para esta unidad.",
        );
      }
    ...
    if (budget.status === "OVERFLOW") {
      return toast.error(
        "NO PERMITIDO: El plan excede la carga horaria máxima de la unidad.",
      );
    }
    ...
    if (selectedGroup) {
      ...
      const stats = selectedGroup.students.reduce(
        (acc, s) => {
          const flags =
            (s.needs || "").toLowerCase() + (s.notes || "").toLowerCase();
    ...
    <div className="w-full max-w-3xl mx-auto bg-white rounded-2xl shadow-xl border border-indigo-50 overflow-hidden">
      ...
      <ul className="steps w-full pt-6 pb-4 px-4 text-sm scale-90">
        ...
        <li
          ...
          onClick={() => setStep(4)}
        >
          {isTechnical ? "Especialidad" : "Materia"}
        </li>
        {isTechnical && (
          <li
        ...
        <li
          className={`step ${step >= (isTechnical ? 6 : 5) ? "step-primary" : ""} cursor-pointer`}
          onClick={() => setStep(isTechnical ? 6 : 5)}
        >
          Unidad
        </li>
        <li
          className={`step ${step >= (isTechnical ? 7 : 6) ? "step-primary" : ""} cursor-pointer`}
          onClick={() => setStep(isTechnical ? 7 : 6)}
        >
          Grupo
        </li>
        <li
          className={`step ${step >= (isTechnical ? 8 : 7) ? "step-primary" : ""} cursor-pointer`}
          onClick={() => setStep(isTechnical ? 8 : 7)}
        >
          Recurso
        </li>
      </ul>
        ...
        {step === 4 && (
          <div className="space-y-6 animate-fade-in-right">
            <div>
              <label className="label font-bold text-slate-700 flex gap-2 items-center">
                <BookOpen size={16} />{" "}
                {isTechnical ? "Especialidad Técnica" : "Asignatura"}
              </label>
              <select
                className="select select-bordered w-full"
                value={data.subject}
          ...
          <div className="space-y-6 animate-fade-in-right">
            <div>
              ...
              >
                ...
                {availableUnitsForSelection.map((u) => (
          ...
          <div className="space-y-6 animate-fade-in-right">
            <div>
              ...
              <div className="mt-4 p-4 bg-blue-50 rounded-lg text-sm text-blue-800 border border-blue-100 flex gap-3 items-start animate-pulse-slow">
                ...
                <div>
                  <strong>Inclusión Automática (DUA):</strong>
                  <br />
                  {data.modality.includes("Adultos") ||
                  data.modality.includes("CINDEA") ||
                  data.modality.includes("IPEC")
        ...
        {step === (isTechnical ? 8 : 7) && (
          <div className="space-y-6 animate-fade-in-right">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-[300px] overflow-y-auto pr-2">
              ...
              <button
                onClick={() =>
                  setData({ ...data, resourceType: "planeamiento" })
                }
              ...
              >
                ...
                <div className="text-xs text-slate-500">
                  Completo (Estrategias + Indicadores)
                </div>
              </button>
              ...
              {isFirstGradePeriod1 ? (
                <button
                  onClick={() =>
                    setData({ ...data, resourceType: "instrumento_sumativo" })
                  }
                ...
                >
                  ...
                  <div className="text-xs text-indigo-600">
                    <strong>NORMATIVA 2024:</strong> Primer Año no realiza
                    prueba escrita en I Periodo.
                  </div>
                </button>
              ) : (
                <button
              ...
              >
                ...
                <div className="text-xs text-purple-600">
                  Ajustado al Calendario Escolar
                </div>
              </button>
              ...
              <button
                onClick={() =>
                  setData({ ...data, resourceType: "practica_pedagogica" })
                }
        ...
        <div className="flex justify-between mt-8 pt-4 border-t border-slate-100">
          ...
          {step < (isTechnical ? 8 : 7) ? (
            ...
            <button
              onClick={submit}
              disabled={
                loading || !data.subject || !data.unit || !data.resourceType
              }
            ...
            >
              {loading ? (
                ...
                <>
                  <Sparkles size={16} /> Generar Recurso
                </>
              )}
            </button>
          )}

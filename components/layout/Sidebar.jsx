...
export function Sidebar() {
  ...
  const menu = [
    ...
    {
      name: "Importador Inteligente",
      icon: UploadCloud,
      path: "/dashboard/tools/importer",
    },
    {
      name: "Cápsulas Familiares",
      icon: Heart,
      path: "/dashboard/tools/family",
    },
    ...
    {
      name: "Configurar Identidad",
      icon: Settings,
      path: "/dashboard/settings/institution",
    },
  ];
  ...
  return (
    <div className="w-64 bg-slate-900 min-h-screen text-slate-300 p-4 flex flex-col">
      ...
      <nav className="flex-1 space-y-1">
        {menu.map((item, i) =>
          item.section ? (
            <div
              key={i}
              className="text-xs font-bold text-slate-500 uppercase mt-6 mb-2 px-2"
            >
              {item.section}
            </div>
          ) : (
      ...
      <div className="p-4 bg-slate-800 rounded-xl mt-4">
        ...
        <div className="text-[10px] text-slate-500">ID: USR-SESSION-ACTIVE</div>
      </div>
    </div>
  );
}

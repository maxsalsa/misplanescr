...
const MEP_SHIELD =
  "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b5/Coat_of_arms_of_Costa_Rica.svg/282px-Coat_of_arms_of_Costa_Rica.svg.png";
...
export const HeaderFormal = ({ institution, teacher, group, resource }) => (
  <header className="w-full border-b-2 border-black mb-6 pb-2 font-serif">
    <div className="flex justify-between items-start mb-4">
      ...
      <div className="text-center flex-1 px-4">
        ...
        <h2 className="font-bold text-md">
          {institution.regionalDirection || "DIRECCIÓN REGIONAL [PENDIENTE]"}
        </h2>
        <h3 className="font-bold text-md">{institution.name}</h3>
        {institution.circuit && (
          <p className="text-sm">Circuito {institution.circuit}</p>
        )}
      </div>
    ...
    <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-sm border-t border-black pt-2">
      <div className="flex">
        <span className="font-bold w-32">Docente:</span> {teacher.name}
      </div>
      <div className="flex">
        <span className="font-bold w-32">Asignatura:</span>{" "}
        {group.specialty || "General"}
      </div>
      <div className="flex">
        <span className="font-bold w-32">Nivel:</span> {group.level}
      </div>
      <div className="flex">
        <span className="font-bold w-32">Curso Lectivo:</span> {group.year}
      </div>
      <div className="col-span-2 flex mt-1">
        <span className="font-bold w-32">Unidad/RA:</span>{" "}
        {resource.linkedOutcome || "General"}
      </div>
    </div>
  </header>
);
  ...
  <header className="w-full mb-6 border-2 border-gray-300 rounded-lg p-4 bg-gray-50">
    ...
    <div className="mt-4 p-3 bg-blue-50 border border-blue-100 rounded text-xs">
      ...
      <p className="mt-1 italic">
        {resource.contentJson?.indicator ||
          "Indicador extraído automáticamente del RAG..."}
      </p>
    </div>
  </header>
);
  ...
  <header className="w-full mb-6">
    ...
    <div className="flex gap-6">
      ...
      <div className="border-2 border-black p-0 w-1/3">
        <div className="bg-gray-200 font-bold text-center border-b border-black text-sm p-1">
          CALIFICACIÓN
        </div>
        <div className="grid grid-cols-2 text-xs divide-x divide-black border-b border-black">
          ...
          <div className="p-1 text-center bg-white font-bold">
            {resource.contentJson?.totalPoints || 45}
          </div>
        </div>
        <div className="grid grid-cols-2 text-xs divide-x divide-black border-b border-black">
        ...
        <div className="grid grid-cols-2 text-xs divide-x divide-black border-b border-black">
          ...
          <div className="p-1 text-center bg-white font-bold">
            {resource.contentJson?.percentage || 25}%
          </div>
        </div>
      ...
      <div className="flex-1 border border-gray-400 p-2 text-sm">
        ...
        <ul className="list-disc pl-4 space-y-1 text-xs">
          ...
          <li>
            No se permite el uso de corrector en las respuestas de selección
            única.
          </li>
...
export const DocumentTemplateSwitcher = ({
  resource,
  institution,
  teacher,
  group,

...
export async function POST(req) {
  try {
    ...
    if (moment.includes("CONEXIÓN") || moment.includes("1")) suggestions = pool.connection;
    else if (moment.includes("CONSTRUCCIÓN") || moment.includes("3")) suggestions = pool.construction;
    ...
    if (studentNeeds === "Visual") responseText += " (Usar apoyo gráfico/video)";
    if (studentNeeds === "Kinestésico") responseText += " (Realizar con material concreto)";
    ...
    return NextResponse.json({ error: "Fallo en el núcleo de inteligencia" }, { status: 500 });
  }
}
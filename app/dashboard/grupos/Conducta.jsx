  ...
  const triggerReport = (student) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (
          type === "GRAVISIMA" ||
          lowerDesc.includes("arma") ||
          lowerDesc.includes("droga") ||
          lowerDesc.includes("suicid")
        ) {
          Swal.fire({
            icon: "warning",
            ...
            footer:
              '<a href="tel:911" style="color:red; font-weight:bold;">LLAMAR AL 9-1-1 AHORA</a>',
          });
          console.log(
            "[QA-AUDIT] TRIGGER FIRED: Risk Content Detected in Conduct Report.",
          );
        } else {
          Swal.fire(
            "Registrado",
            "La boleta ha sido guardada en Neon DB.",
            "success",
          );
        }
      }
  ...
  return (
    <div>
      <div className="alert alert-warning mb-6 text-black">
        <AlertTriangle />
        <span>
          <strong>Protocolo Activo:</strong> Cualquier reporte de armas o drogas
          disparará alerta inmediata (9-1-1).
        </span>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div
          onClick={() => triggerReport("Max Salazar")}
          className="bg-slate-800 p-4 rounded-lg flex justify-between items-center hover:bg-slate-700 cursor-pointer border border-transparent hover:border-red-500 transition-all"
        >
        ...
        <div
          onClick={() => triggerReport("Sofia Vargas")}
          className="bg-slate-800 p-4 rounded-lg flex justify-between items-center hover:bg-slate-700 cursor-pointer border border-transparent hover:border-red-500 transition-all"
        >

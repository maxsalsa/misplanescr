...
export default function AnnualPlannerPage() {
  ...
  const [units, setUnits] = useState([
    {
      id: 1,
      name: "Unidad 1: Fundamentos TI",
      officialHours: 96,
      plannedHours: 96,
    },
    {
      id: 2,
      name: "Unidad 2: Seguridad Industrial",
      officialHours: 36,
      plannedHours: 36,
    },
    {
      id: 3,
      name: "Unidad 3: Electricidad y Electrónica",
      officialHours: 28,
      plannedHours: 28,
    },
  ]);
  ...
  const handleSave = () => {
    if (totalPlanned < totalOfficial) {
      toast.error("Plan Incompleto", {
        description: "Debe cubrir el 100% de horas oficiales.",
      });
    } else {
      toast.success("Plan Anual Validado", {
        description: "Cobertura curricular del 100% garantizada.",
      });
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Distribución de horas según Programa de Estudio (Cobertura Total).
          </p>
        </div>

        {/* INDICADOR DE CUMPLIMIENTO */}
        <div
          className={`px-6 py-3 rounded-xl border-2 font-bold text-lg flex items-center gap-3 ${totalPlanned >= totalOfficial ? "border-blue-100 bg-blue-50 text-blue-700" : "border-yellow-100 bg-yellow-50 text-yellow-700"}`}
        >
          <CheckCircle2 size={24} />
          <span>
            Cobertura: {Math.round((totalPlanned / totalOfficial) * 100)}%
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="flex justify-end">
            <button
              onClick={handleSave}
              className="btn btn-institutional w-full md:w-auto"
            >
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">
                  Horas Calendario (aprox):
                </span>
                <span className="font-bold text-yellow-400">
                  {physicalAvailability} h
                </span>
              </div>
              ...
              <div>
                ...
                <progress
                  className={`progress w-full mt-2 ${intensity > 100 ? "progress-warning" : "progress-success"}`}
                  value={intensity}
                  max="120"
                ></progress>
              </div>
              ...
              <div className="bg-white/10 p-3 rounded-lg text-xs leading-relaxed mt-4">
                <strong>Nota Técnica:</strong> Debido a la capacitación de
                febrero, el ritmo de clase debe acelerarse un{" "}
                <strong>{Math.round(intensity - 100)}%</strong> para cubrir el
                100% de contenidos sin recortar el programa.
              </div>
            </div>
          </div>
        </div>

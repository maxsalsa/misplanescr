...
export default function SubscriptionManager() {
  ...
  const handleActivate = (id) => {
    ...
    const confirmed = window.confirm(
      "¿Ha verificado el comprobante SINPE por ₡3,500?",
    );
    if (!confirmed) return;
          ...
          <tbody className="text-slate-700">
            {users.map((user) => (
              <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td>
                  <span
                    className={`badge ${user.status === "ACTIVE" || user.status === "SUPER_ADMIN" ? "badge-success text-white" : "badge-warning text-white"}`}
                  >
                    {user.status}
                  </span>
                </td>

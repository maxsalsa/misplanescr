...
export default async function GroupsPage() {
  ...
  const where =
    session?.user?.role === "GOD_TIER" ? {} : { userId: session?.user?.id };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Administre sus secciones y listas de estudiantes.
          </p>
        </div>
        <Link
          href="/dashboard/groups/new"
          className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-6 rounded-lg shadow-lg shadow-purple-200 transition-all"
        >
          + Nuevo Grupo
        </Link>
      </div>
      ...
      {groups.length === 0 ? (
        <div className="text-center py-20 bg-slate-50 rounded-xl border-dashed border-2 border-slate-200">
          ...
          <p className="text-slate-400 mb-6">
            Comience creando su primer grupo de estudiantes.
          </p>
        </div>
      ) : (

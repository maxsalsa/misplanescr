...
export default function RequestsPage() {
  ...
  const [requests, setRequests] = useState([
    {
      id: 1,
      student: "Mora Rojas Sebastián",
      type: "Justificación Médica",
      date: "27/01/2026",
      status: "PENDIENTE",
      file: "dictamen.pdf",
    },
    {
      id: 2,
      student: "Jiménez Castro Valentina",
      type: "Permiso de Salida",
      date: "28/01/2026",
      status: "APROBADO",
      file: null,
    },
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-6 animate-in fade-in">
        <div className="bg-white p-8 rounded-2xl shadow-lg border-t-4 border-blue-600">
          ...
          <p className="text-slate-500 mb-6">
            Envíe justificaciones o solicite permisos para su hijo/a.
          </p>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:bg-slate-50 cursor-pointer">
                ...
                <span className="text-xs text-slate-500">
                  Clic para subir dictamen o carta
                </span>
              </div>
            </div>
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Bandeja de Entrada de Trámites Oficiales.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          {requests.map((req) => (
            <div
              key={req.id}
              className="card-solemn flex justify-between items-center group hover:border-blue-300 cursor-pointer transition-all"
            >
              <div className="flex items-start gap-4">
                <div
                  className={`p-3 rounded-full ${req.status === "PENDIENTE" ? "bg-orange-100 text-orange-600" : "bg-emerald-100 text-emerald-600"}`}
                >
              ...
              {req.status === "PENDIENTE" && (
                <div className="flex gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => toast("Solicitud Rechazada")}
                  ...
                  <button
                    onClick={() => toast.success("Solicitud Aprobada")}
                    className="btn btn-circle btn-sm btn-ghost text-emerald-500 hover:bg-emerald-50"
                    title="Aprobar"
                  >
                    <CheckCircle size={20} />
                  </button>
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-lg">
            ...
            <p className="text-xs text-slate-400 mb-4">
              Envíe un acceso seguro para gestión de trámites.
            </p>

...
export default function CreateActivityPage() {
  ...
  const addCriterion = () => {
    if (!newCriterion.indicator || !newCriterion.initial || !newCriterion.advanced) {
      toast.error("Rigor Faltante", { description: "Debe definir el indicador y los 3 niveles de desempeño." });
      return;
    }
  ...
  const handleSave = () => {
    if (criteria.length === 0) {
      toast.error("Evaluación Inválida", { description: "IMPOSIBLE GUARDAR: Falta la Rúbrica Analítica." });
      return;
    }
    toast.success("Actividad Blindada Guardada", { description: `Registrada con éxito bajo normativa V140.` });
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20 p-6 animate-in fade-in">
      ...
      <div className="flex items-center gap-3 border-b border-slate-200 pb-4">
        ...
        <div>
           ...
           <p className="text-slate-500 text-sm">Protocolo de Rigor V140: Rúbricas Obligatorias.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <div className="card-solemn p-6 h-fit">
            ...
            <div className="alert bg-orange-50 text-orange-800 text-[10px] flex items-start gap-2">
               ...
               <span>Sin Rúbrica, no hay actividad válida (Reglamento de Evaluación).</span>
            </div>
         </div>

...
export default function GradebookTab({ section }) {
    ...
    const getIcon = (type) => {
        if (type === "JUEGO") return <Gamepad2 size={16} className="text-purple-500" />;
    ...
    const saveColumn = async (activityId) => {
        ...
        section.students.forEach(s => {
            ...
            if (key in editingGrade) {
                updates.push({ studentId: s.id, score: parseFloat(editingGrade[key]), feedback: "" });
            }
        });
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-lg font-bold text-slate-700">Registro de Calificaciones</h2>
                <button className="btn btn-primary btn-sm gap-2" onClick={() => dialogRef.current.showModal()}>
            ...
            <div className="overflow-x-auto border border-slate-200 rounded-xl shadow-sm">
                <table className="table">
                    <thead className="bg-slate-50 text-slate-700">
                        <tr>
                            <th className="sticky left-0 bg-slate-50 z-10 w-48">Estudiantes</th>
                            {activities.map(act => (
                                <th key={act.id} className="min-w-[120px] text-center group">
                                    <div className="flex flex-col items-center">
                                        ...
                                        <span className="text-[10px] font-normal text-slate-500 truncate max-w-[100px]" title={act.title}>{act.title}</span>
                                        <div className="mt-1 badge badge-ghost badge-sm">{act.percentage}%</div>
                                        <button onClick={() => saveColumn(act.id)} className="btn btn-xs btn-ghost text-emerald-600 mt-1 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <tbody>
                        {section.students.map(student => (
                            <tr key={student.id} className="hover:bg-slate-50">
                                ...
                                {activities.map(act => {
                                    // Find existing grade
                                    const gradeObj = act.grades.find(g => g.studentId === student.id);
                                    const key = `${student.id}-${act.id}`;
                                    const currentVal = key in editingGrade ? editingGrade[key] : (gradeObj?.score ?? "");
                ...
                <div className="modal-box">
                    ...
                    <form action={handleCreate} className="space-y-3">
                        ...
                        <div className="form-control">
                            ...
                            <input name="title" className="input input-bordered" placeholder="Ej: Trivia Volcanes" required />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="form-control">
                                ...
                                <input name="percentage" type="number" className="input input-bordered" placeholder="5" required />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="form-control">
                                ...
                                <input name="maxPoints" type="number" className="input input-bordered" defaultValue="100" />
                            </div>
                            <div className="form-control">
                                <label className="label">Fecha</label>
                                <input name="date" type="date" className="input input-bordered" defaultValue={new Date().toISOString().split('T')[0]} />
                            </div>
                        </div>

                        <div className="form-control">
                            <label className="label">Indicador / Justificación</label>
                            <textarea name="indicator" className="textarea textarea-bordered" placeholder="Habilidad evaluada..." required></textarea>
                        </div>

                        <div className="modal-action">
                            <button className="btn btn-primary">Crear</button>
                            <button type="button" className="btn" onClick={() => dialogRef.current.close()}>Cancelar</button>
                        </div>
                    </form>
                </div>
            </dialog>

...
export async function ingestMepPrograms() {
  ...
  if (session?.user?.role !== "SUPER_ADMIN") {
    throw new Error("⛔ ACCESO DENEGADO: Protocolo reservado para Arquitectos.");
  }
  ...
  try {
    ...
    if (count === 0) {
      ...
      await prisma.mep_programs_core.createMany({
        data: [
          {
            program_name: "Matemáticas - I Ciclo",
            grade_level: "1-3",
            raw_content: "Logica...",
          },
          {
            program_name: "Español - II Ciclo",
            grade_level: "4-6",
            raw_content: "Gramatica...",
          },
          {
            program_name: "Ciencias - III Ciclo",
            grade_level: "7-9",
            raw_content: "Biologia...",
          },
        ],
      });
    }
    ...
    return {
      success: true,
      message: "Protocolo de Ingesta Completado. Neon DB Sincronizada.",
    };
  } catch (error) {
    console.error("❌ Error en ingesta:", error);
    return {
      success: false,
      message: "Fallo en el núcleo de procesamiento: " + error.message,
    };
  }
}

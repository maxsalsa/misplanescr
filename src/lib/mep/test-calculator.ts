 ...
 *    * Nota: JS Math.round() hace esto estándar (x.5 sube), pero implementamos lógica explícita para seguridad.
 */
...
export interface TestGridResult {
    totalLessons: number;
    constantK: number;
    distribution: GridRow[];
    calculatedPoints: number; // Verification sum
}

export function calculateTestGrid(input: TestInput): TestGridResult {
    // 1. Sum Total Lessons
    ...
    const distribution: GridRow[] = input.indicators.map(ind => {

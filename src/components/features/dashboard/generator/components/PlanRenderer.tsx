...
export default function PlanRenderer({ plan, currentVersion, versions = [], onUpdate, readOnly = false }: PlanProps) {
      ...
      `}>
                ...
                <div className="p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <div className="flex flex-wrap gap-2 justify-end max-w-md">
                        {/* Logic for Values */}
                        {encabezado?.valores?.map((val: string, i: number) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                {/* COL 1 & 2: CURRICULAR MATRIX (Span 5/12) */}
                        ...
                        <CardContent className="p-0">
                            {(!columnas || columnas.length === 0) ? (
                                ...
                                <div className="divide-y divide-slate-100">
                                    {columnas.map((col: any, idx: number) => (
                                            ...
                                            <div className="p-5 bg-white">
                                                ...
                                                <ul className="space-y-2">
                                                    ...
                                                    {Array.isArray(col.saberes)
                                                        ? col.saberes.map((s: string, i: number) => <SaberItem key={i} text={s} color={themeColor} />)
                ...
                {/* COL 3: MEDIATION STRATEGIES (Span 7/12 - "La más ancha") */}
                <div className="lg:col-span-7 space-y-4">
                    {!mediacion ? (
                        ...
                        mediacion.map((fase: any, idx: number) => (
                            <MediationCard
                                key={idx}
                                fase={fase}
                                idx={idx}
            ...
            <div className="text-center pt-8 pb-4">
                <p className="text-xs text-slate-300 uppercase tracking-widest flex items-center justify-center gap-2 opacity-70">
                    <ShieldCheck className="w-4 h-4" /> Estructura Validada por AutoPlanea MEP
                </p>
            </div>
...
function SaberItem({ text, color }: { text: string, color: string }) {
    return (
...
function MediationCard({ fase, idx, themeColor, readOnly }: any) {
...
function IronLawGuard({ content }: { content: string }) {

...
export const INSTRUMENT_TYPES = {
  RUBRIC: {
    ...
    description:
      "Para evaluar niveles de desempeño progresivo (Inicial, Intermedio, Avanzado). Obligatorio para Procesos.",
    defaultScales: [
      "Inicial (1 pt)",
      "Intermedio (2-3 pts)",
      "Avanzado (4-5 pts)",
  ...
  CHECKLIST: {
    ...
    description:
      "Verificación dicotómica (Sí/No). Ideal para seguridad, procedimientos y producto terminado.",
  ...
  SCALE: {
    ...
    description:
      "Frecuencia o grado de una conducta (Siempre, A veces, Nunca).",
  ...
  PORTFOLIO: {
    ...
    description:
      "Instrumento integrador para verificar la colección de evidencias del periodo.",
    defaultCriteria: [
      "Organización y Limpieza",
      "Completitud de Evidencias",
...
export function validateInstrument(instrument) {
  if (!instrument.criteria || instrument.criteria.length === 0) {
    return {
      valid: false,
      error: "❌ ILEGAL: El instrumento no tiene indicadores.",
    };
  }
  if (!instrument.totalPoints || instrument.totalPoints <= 0) {
    return {
      valid: false,
      error: "❌ ERROR: El puntaje total debe ser mayor a 0.",
    };
  }
  return { valid: true };
}

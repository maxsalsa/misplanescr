...
export function generateMultiverse(
  unitContext,
  diagnosticData,
  userTier = "FREE",
) {
  ...
  const diagAnalysis = analyzeDiagnosticState(
    diagnosticData || { score: 80, total: 100 },
  ...
  suite.resources.plan = {
    ...
    content: {
      ...
      rubric: isHighPot
        ? "Rúbrica de 4 niveles (incluye 'Experto')."
        : "Rúbrica estándar 1-3.",
      adaptation: isRemedial
        ? "Simplificación de consignas."
  ...
  suite.resources.gta = {
    ...
    structure: {
      preparo:
        "Materiales: Cuaderno, lápiz, dispositivo móvil. ambiente silencioso.",
      recordar: isRemedial
        ? "Repaso de conceptos clave con ejemplos visuales."
        : "Activación de conocimientos previos mediante preguntas divergentes.",
      practica: isHighPot
        ? "Resolución de problemas de caso complejo."
  ...
  suite.resources.quiz = {
    title: isRemedial
      ? "Tiquete de Salida (Verificación)"
      : "Trivia de Competencia (Kahoot)",
    type: "QUIZ",
    items: [
  ...
  if (userTier !== "ULTRA") {
    Object.keys(suite.resources).forEach((key) => {
      suite.resources[key].watermark =
        "PREVIEW ONLY - UPGRADE TO AULAPLAN ULTRA";

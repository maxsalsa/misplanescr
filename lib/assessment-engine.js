...
export const calculateTableSpecs = (
  topics,
  totalPoints = 100,
  totalTimeOverride = null,
) => {
  ...
  if (!totalTime) {
    totalTime = topics.reduce(
      (sum, t) => sum + (t.minutes || (t.lessons || 1) * 40),
      0,
    );
  }
...
export const validateETPStructure = (planJson) => {
  ...
  if (
    !planJson.columns?.some(
      (c) => c.key === "time" || c.label.includes("Tiempo"),
    )
  ) {
    errors.push(
      "CRITICAL: ETP Plans MUST include 'Tiempo Estimado' column (Orientaciones ETP).",
    );
  }

  // Check for "Resultados de Aprendizaje" (RA)
  if (
    !planJson.sections?.some(
      (s) => s.type === "RA" || s.title.includes("Resultado"),
    )
  ) {
    errors.push(
      "CRITICAL: ETP Plans MUST utilize 'Resultados de Aprendizaje' instead of just 'Objetivos'.",
    );
  }
...
export const getTechnicalWeights = (specialty, level) => {
  ...
  if (
    specialty &&
    (specialty.includes("Web") || specialty.includes("Informática"))
  ) {

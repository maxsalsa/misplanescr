...
const sendRecoveryEmail = async (email, code) => {
  console.log(
    `📧 [EMAIL-SEC] To: ${email} | Subject: 🆘 Código de Recuperación`,
  );
  console.log(
    `   Body: "Su código temporal es: [ ${code.split("").join(" ")} ] (Expira en 15m)"`,
  );
};
...
export async function requestPasswordReset(email) {
  ...
  if (!user) {
    console.log(
      `   ⛔ Email no encontrado. Simulando éxito para evitar enumeración.`,
    );

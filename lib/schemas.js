...
export const criterionSchema = z.object({
  text: z
    .string()
    .min(5, "El nombre del criterio debe ser descriptivo (min 5 letras)"),
  points: z
    .number()
...
export const writtenTestSchema = z.object({
  criteria: z
    ...
    .min(1, "Debe agregar al menos un criterio"),
  totalPoints: z
    .number()
    .max(100, "La suma total no puede exceder el 100%")
    .refine((val) => val > 0, {
...
export const hrRequestSchema = z
  .object({
    ...
    justification: z
      .string()
      .min(10, "La justificación debe ser detallada (min 10 chars)"),
  ...
  .refine(
    (data) => {
      ...
      if (
        (data.type === "ABSENCE" || data.type === "LICENSE") &&
        !data.evidenceUrl
      ) {
        return false;
      }
    ...
    {
      message:
        "Para Ausencias o Licencias, el comprobante digital es OBLIGATORIO.",
      path: ["evidenceUrl"],
    },
  );

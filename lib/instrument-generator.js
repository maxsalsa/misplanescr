...
export const generateSummativeInstrument = (data) => {
  ...
  doc.setFontSize(10);
  doc.text("MINISTERIO DE EDUCACIÓN PÚBLICA", pageWidth / 2, 15, {
    align: "center",
  });
  doc.text(
    `DIRECCIÓN REGIONAL DE EDUCACIÓN: ${data.regional || "__________"}`,
    pageWidth / 2,
    20,
    { align: "center" },
  );
  doc.text(
    `CENTRO EDUCATIVO: ${data.institutionName || "__________"}`,
    pageWidth / 2,
    25,
    { align: "center" },
  );
  ...
  doc.text(
    `Periodo: I Periodo (Evaluación Sumativa)`,
    pageWidth / 2 + 10,
    yPos,
  );
  yPos += lineHeight;
  ...
  doc.text("INSTRUMENTO DE EVALUACIÓN SUMATIVA", pageWidth / 2, yPos, {
    align: "center",
  });
  doc.setFontSize(10);
  doc.setTextColor(100);
  doc.text(
    "(Sustituye a la Prueba Escrita en I Periodo de Primer Año)",
    pageWidth / 2,
    yPos + 6,
    { align: "center" },
  );
  ...
  const indicators = data.indicators || [
    {
      indicator: "Reconoce vocales en palabras simples.",
      criteria: "Identifica a/e/i/o/u",
    },
  ...
  doc.text("SELLO COMITÉ DE EVALUACIÓN", margin + 30, yPos + 20, {
    align: "center",
  });
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(100);
    doc.text(
      "Documento validado por la tecnología de MisPlanesCR (Lic. Max Salazar Sánchez - 60906359)",
      pageWidth / 2,
  ...
  // Save
  doc.save(
    `Instrumento_Sumativo_${data.subject}_${data.group || "General"}.pdf`,
  );
};
...
export const generateDiagnosticInstrument = (data, type) => {
  ...
  doc.setFontSize(10);
  doc.text("INSTRUMENTO DE EVALUACIÓN DIAGNÓSTICA", pageWidth / 2, 22, {
    align: "center",
  });
  ...
  if (type === "Lista de Cotejo (Checklist)") {
    ...
    doc.autoTable({
      startY: yPos,
      head: [
        ["Estudiante", "Fecha/Hora", "Descripción del Hecho", "Interpretación"],
      ],
      body: tableBody,
      theme: "grid",
  ...
  doc.setTextColor(150);
  doc.text(
    "Validado por Antigravity Prime (MEP 2026 Compatible)",
    pageWidth / 2,
    pageHeight - 10,
    { align: "center" },
  );

...
export function generateTechnicalPlan(specialtyKey, moduleName, raText) {
  ...
  // Safety Phase Logic
  let safetyPhase =
    "Charla de Seguridad: Ergonomía frente al computador (5 min).";
  if (specialty.risk === "HIGH") {
    safetyPhase =
      "PROTOCOLO ROJO: Verificación de EPP (Gafas, Botas, Guantes) + Charla de Seguridad de 5 min.";
  ...
  return {
    ...
    phases: {
      ...
      desarrollo:
        "Práctica Guiada: El estudiante ejecuta el procedimiento bajo supervisión (Aprender Haciendo).",
...
export function generateProductRubric(productName) {
  return {
    ...
    criteria: [
      {
        name: "Acabado / Presentación",
        levels: ["Excelente", "Bueno", "Deficiente"],
      },
      {
        name: "Funcionalidad",
        levels: ["Opera 100%", "Fallo Menor", "No Funciona"],
      },
      {
        name: "Tiempo de Entrega",
        levels: ["A tiempo", "Retraso Justificado", "Tardío"],
      },
      {
        name: "Uso de Materiales",
        levels: ["Eficiente", "Desperdicio Mínimo", "Desperdicio Alto"],
      },
    ],
  };
}

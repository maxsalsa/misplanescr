...
export async function generateApexSuite(request, userContext, dbClient) {
  ...
  const zenithResult = await handleZenithRequest(
    request.query,
    userContext,
    dbClient,
  );
  ...
  const plan = {
    ...
    phases: {
      ...
      desarrollo:
        "Estaciones de Aprendizaje: Rotación por 3 centros (Investigación, Práctica, Creación).",
      cierre:
        "Tiquete de Salida: Metacognición con Emojis (¿Qué aprendí hoy?).",
    },
    ...
    evaluation: {
      ...
      admin_note:
        "Recuerde llenar la Boleta de Asistencia y revisar Adecuaciones (IEP).",
    },
  };

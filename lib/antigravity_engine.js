...
export async function generateClassPlan(request) {
  ...
  // 3. EL PROMPT MAESTRO
  const prompt = `
    ROL un Asesor Pedagógico experto del MEP Costa Rica.
    OBJETIVOñar un plan de "${request.planType}" realista y detallado.
  ...
  const savedPlan = await prisma.pedagogicalPlan.create({
    data: {
      ...
      rubricJson.stringify(jsonResponse.rubric)
    }
  });
